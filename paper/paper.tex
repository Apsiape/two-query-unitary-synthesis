...
% Content matches paper/paper.md version 1.0.2 (the markdown is the GitHub-rendered copy).
...
up to the factor $\log(1/\varepsilon)$. We also show that the bound is tight at workspace
$\Theta(N^2)$, explain why the one-query argument does not extend, and prove a transfer
theorem reducing the garbage-model two-query problem to a clean four-insertion width bound,
which remains open; no width argument that is linear in the realized operator and blind to
the junk can do with fewer insertions.
\end{abstract}

\begin{callout}{Verification note.}
Every numbered claim below that admits a finite check is exercised by a script in
\texttt{verify/} of the accompanying repository, \url{https://github.com/Apsiape/two-query-unitary-synthesis}, and
\texttt{STATUS.md} there records, claim by claim, which checks exist, what has been
independently audited and what has not. File names in this paper are paths in that
repository.
\end{callout}
...
unitaries, at most $2N$ oracle bits used;
...
$\poly(n)$ ancillas.
\end{itemize}
Against two or more \emph{adaptive} queries no capacity bound was known; on the other side
there were only the trivial lookup and the permutation synthesizer of \cite{DLM26}. This
paper gives, to our knowledge, the first capacity bound at two adaptive queries and shows
it is essentially tight.

\subsection{The model, and why ``clean'' is the right restriction}
...
requirement, and our lower bounds hold without it. The only known one-query universal
synthesizer, the Bernstein--Vazirani lookup attributed to Yuen in \cite{LMW24}, is not
...
Verified exhaustively in \texttt{verify\_decisive\_pair.py}: exactness residuals below
$10^{-14}$; the first invariant to differ is the last frame, $\|K_3^A-K_3^B\|=2.00$. The same
script checks Proposition~\ref{prop:invisible} on two-query architectures with non-empty
lawful sets under several final layers.
...
tool to count; the second says that beyond depth two even that reduction fails, so any
bound computed from pre-final data alone is at best a maximum over completions. Both point
...
\begin{proof}
Six steps; the first three are exact identities.
...
instance. The deterministic spine of the symmetric route
is formalized in the \texttt{lean/} directory of the repository.
...
Sudakov--Fernique inequality \cite[Thm~7.2.11]{Ver18}, and using that the expected maximum
of $m\ge2$ independent standard normals is at least $0.677\sqrt{\log m}$ (the minimum over
$m$ is at $m=2$, where the ratio is $1/\sqrt{\pi\log2}$), gives
$\epsilon\sqrt{\log\Pack}\le2.09\,\E\sup X_T$. Hence $\kappa=8C_S^2\le35$ for the constant-2
route of Theorem~\ref{thm:width}, and $\kappa\le560$ for the audited constant-8 form. Either
way the requirement $\kappa\ge1/4$ is met with two orders of magnitude to spare. We record
the requirement because it is real: at $\kappa<1/4$ the theorem would be contradicted by a
published construction.

\section{Tightness, address length, and the matching upper bound}\label{sec:six}

\subsection{Tight at workspace of order \texorpdfstring{$N^2$}{N squared}}\label{sec:tight}
...
repository note \nolinkurl{notes/permutation-transport.md}.
...
their one-query bounds there. This section prices, exactly, what a two-query bound in that
model would require of the clean machinery.
...
multilinear polynomials of degree $t$ in the signs. In the \textbf{keep form}
...
The width-and-Sudakov machinery of Sections~\ref{sec:width} and \ref{sec:capacity} prices a
Gaussian process that is \emph{linear} in the realized operator. By the lemma, no junk-blind
linear functional of a garbage packet is nonzero; the invariants that factor through the
channel are quadratic, the basic one being $S_g^{*}(X\otimes I)S_g=U_g^{*}XU_g$, of degree
$2t$ in the signs. This is the exact sense in which cleanliness halves the degree, and it
says what any junk-blind width argument, linear in the realized operator, for garbage-$t$
must be: a clean bound at $2t$ insertions.

\begin{theorem}[conjugation transfer]\label{thm:transfer}
Consider the $2t$-insertion class $\{Y_{2t}D_gY_{2t-1}\cdots D_gY_0:\|Y_i\|\le1\}$ with
...
Suppose it satisfies a capacity bound $\log\Pack_{\rho\sqrt N}\le C_\rho N\,\phi(Q)$ for some
function $\phi$ of the address count. Then no universal garbage-$t$ synthesizer exists with
$\phi(Q)=o(N)$, for every workspace and junk dimension, with no change to the oracle, at
constant diamond error. At $t=1$, Theorem~\ref{thm:address} gives $\phi(Q)=\log(2QN)$.
\end{theorem}
...
point has Frobenius norm $\sqrt N$; volume counting there gives, for $\rho$ below an
absolute constant, a $\rho\sqrt N$-separated set of $e^{c_\rho N^2}$ points, each of the form
$U_i^{*}RU_i$. A universal synthesizer must in particular synthesize $U_1,\dots,U_m$, so the
single family $\{T_g\}$ contains an $e^{c_\rho N^2}$-point separated code, contradicting the
assumed capacity bound when $\phi(Q)=o(N)$. The discard form costs an additional
$O(\sqrt\varepsilon)$ in normalized Frobenius distance, absorbed by halving $\rho$.
\end{proof}
...
subclass $Y_g^{*}XY_g$ with $Y_g$ a two-insertion packet whose left endpoint is the identity
...
the Aaronson--Kuperberg question negatively. Less would do: any bound of the form
...
with address width $2N$, independent of $Q$. The address-width concept is theirs.
\item \textbf{A remark on framing.} Quantum query algorithms are exactly the completely
bounded multilinear forms, by the Christensen--Sinclair factorization \cite{ABP18}. In that
language a two-query architecture is a completely bounded bilinear map from the selector
torus to $N\times N$ matrices, Theorem~\ref{thm:width} bounds the Gaussian mean width of its
range uniformly over such maps, and the $k$-query conjecture concerns the ranges of
$k$-linear maps. We record this as a reformulation, not a result.
\end{enumerate}
...
\texttt{verify/run\_all.py} in the repository runs eight deterministic, numpy-only scripts,
each exiting nonzero on any failed check: Theorem~\ref{thm:width} in both forms (exact
expansion, pointwise absorption over the whole cube and the torus, the closed-form variance
...
The \texttt{lean/} directory holds a kernel-checked formalization of the deterministic spine
...
included scripts; the audit status of each claim is recorded in \texttt{STATUS.md}.
